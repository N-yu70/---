...
function findFood(foodName) {
  ...
  for (let i = 0; i < keys.length; i++) {
    if (name.includes(keys[i]) || keys[i].includes(name)) {
      return { key: keys[i], data: FOOD_DB[keys[i]] }
    }
  }
  return null
}
...
function estimateCalories(foodName, amount, unit) {
  ...
  if (found) {
    const { data } = found
    if (data.perUnit && data.perUnit[u]) {
      return Math.round(data.perUnit[u] * amt)
    }
    if (u === 'g' && data.per100g) {
      return Math.round((data.per100g * amt) / 100)
    }
    if (data.per100g) {
  ...
  if (u === 'g') {
    return Math.round((DEFAULT_PER_100G * amt) / 100)
  }
...
function recommendTarget(profile) {
  ...
  const factor = factors[profile.activityLevel] || 1.375
  return Math.round(bmr * factor)
}

function getStatus(intake, target) {
  if (!target || target <= 0) {
    return { level: 'unknown', text: '请先设置热量目标', color: '#999' }
  }
...
function getMockAdvice(intake, target, profile) {
  const status = getStatus(intake, target)
  const pref = (profile.preferences || '').trim()
  if (status.level === 'low') {
    return `今日摄入偏少。${pref ? '结合你的偏好「' + pref + '」，' : ''}可补充优质蛋白和全谷物，例如鸡蛋、酸奶或一小碗燕麦。`
  }
  if (status.level === 'high') {
    return `今日热量已偏高。晚餐建议选择清淡蔬菜与瘦肉，少喝含糖饮料；${pref ? '也记得你的偏好：' + pref + '。' : ''}`
  }
  if (status.level === 'ok') {
    return `摄入较均衡，继续保持！${pref ? '已记录你的饮食偏好「' + pref + '」。' : ''}下一餐注意荤素搭配即可。`
  }
  return '在「我的」中填写身高体重，系统会为你推荐每日热量目标。'
}
